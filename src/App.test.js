...
describe('List'  , () => {
  it('should render This list is empty inside a paragraph component if items list is an empty array' , () => {
...
describe('testing the Test Component', () => {
  ...
  it('has the initial state count of zero', () => {
    expect(wrapper.state()).toEqual({ count: 0, name: ''});
  })

  it('increments by 1 on one click', () => {
    wrapper.find('#count-up').props().onClick();
    expect(wrapper.state()).toEqual({ count: 1, name: ''});
  })
...
describe('hook component useState', () => {
  let wrapper;
